...
This vignette will walk through some functions in clinfun that are available to help with designing phase II clinical trials. 

We will use data examples from (citations)
...
Clinfun can help determine necessary sample sizes and number of responses needed for treatments to be considered desirable in stage 2 designs. 
...
When designing a Simon's two-stage trial, the key objective is to select appropriate values of response thresholds indicated by:
...
The response rate considered "unacceptable" (at which point a treatment would not be recommended) and the response rate considered "desired" must be specified, and the values of $r_1$, $r$, $n_1$, and $n$ must be selected such that the desired error rate constraints are satisfied.
