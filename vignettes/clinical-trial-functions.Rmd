...
Stopping rules in clinical trials are a useful tool for risk management and can help ensure the protection of participants. Patient outcomes are monitored over the course of the trial, and the trial is terminated if certain conditions are met (eg, if an unacceptable rate of toxicity is observed). Stopping rules define the specific conditions under which the trial should be terminated.

The most common types of stopping rules include:

- **Stopping for toxicity:** The trial is stopped if some unacceptable level of toxicity is observed. Terminating the trial minimizes the number of patients exposed to a treatment that is likely too toxic relative to its potential benefits.

- **Stopping for futility:** The trial is stopped is some unacceptable level of efficacy is observed. Terminating the trial minimizes the number of patients exposed to a treatment that likely does not provide adequate benefit relative to its risks.

- **Stopping for efficacy:** The trial is stopped if some striking level of efficacy is observed. Terminating the trial expedites the development of a treatment for which there is already compelling evidence that its benefits outweigh its risks.

A trial can implement multiple types of stopping rules. The choice of stopping rules is highly dependent on factors such as disease area, the clinical outcomes of interest, stage of development, and characteristics of the treatment itself.

This section describes the specific types of stopping boundaries that can be constructed using {clinfun}.

[Note: if we include a section on group sequential designs, maybe we could reference that and mention that early stopping can be implemented via group sequential designs but that's a separate thing?]

## Stopping for Toxicity (For Phase II and Phase III Designs)

`toxbdry()` computes a stopping rule for toxicity monitoring that can be implemented in phase II and III designs. For example, a stopping rule generated by this function could be layered on top of a design generated by `ph2simon()` or `ph2single()`.

## Stopping for Futility (For Phase II Single-Stage Designs)

`futilbdry()` computes a stopping rule for futility monitoring that is specifically intended to be implemented in phase II single-stage designs. Thus, a stopping rule generated by this function would be layered on top of a design generated by `ph2single()`.

A single-stage design with early stopping for futility is typically chosen instead of a two-stage design when the response outcome takes a long time to assess and/or accrual for trial is slow. In these scenarios, assessing efficacy on a continual basis is preferable to waiting for completion of the first of two stages.
