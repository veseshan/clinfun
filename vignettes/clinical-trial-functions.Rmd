...
This vignette will walk through functions in clinfun that are available to help with designing phase II clinical trials. We will use data examples from (citations) to demonstrate examples in the context of real studies. 

Before going through the vignette, please install and load {clinfun}:
...
The overall purpose of phase II research is to determine whether a treatment is promising enough to warrant more extensive development. While phase I research assesses safety and clinical pharmacology, phase II research begins assessing the efficacy of a treatment in a well-defined population, while continuing to monitor the safety of the treatment. {clinfun} can help determine necessary sample sizes and number of responses needed for treatments to be considered desirable in stage 2 designs. 

## Simon's Two-Stage

Simon's two-stage design is one type of phase II design that aims to assess the efficacy of the treatment while minimizing the number of patients exposed to an ineffective treatment.
...
When designing a Simon's two-stage trial, the key objective is to determine appropriate values of response thresholds indicated by:

  - **$r_1$** (response threshold at the end of stage 1)
  - **$r$** (final response threshold)

and sample sizes, indicated by:
...
The values of the response thresholds depend on what response rate is considered "unacceptable" ($p_0:$; at which point a treatment would not be recommended),  what response rate is considered "desired" ($p_1:$) as well as the desired error rate constraints ($\alpha:$, $\beta:$) needed to test the following hypotheses:
...
{clinfun} will need these parameters to determine final design thresholds, therefore the following parameters and constraints must be specified: 

- `pu`- $p_0:$ Treatment response rate considered unacceptable

- `pa`- $p_1:$ Treatment response rate considered desirable

- `eps1`- $\alpha:$ Type I error threshold

- `eps2` - $\beta:$ Type II error threshold

When these parameters are supplied, the `ph2simon()` function can return the required response thresholds: 

```{r}
ph2simon(pu = 0.2, pa = 0.5, ep1 = 0.1, ep2 = 0.1)
```

The results above indicate that under these constraints, the treatment is considered desirable if 3 or more responses are seen (`> r1`). The trial should be stopped early if 2 or fewer responses are seen (`≤ r1`).

### Optimal vs. Minimax vs. Admissible Designs

In the results above, you will notice two rows of results: `Optimal` and `Minmax`. These give you threshold values under two possible designs that differ in treatment of expected sample size. 
...
The trial employed a Simon's two-stage design. An overall response rate of 5% was considered unacceptable, and an overall response rate of 20% was considered desired. The  type I error threshold used was 10%. The target power used was 90%, equivalent to a 10% type II error threshold.

Summarizing the design parameters and constraints from above, we have:

* $p_0:$ 5% overall response rate

* $p_1:$ 20% overall response rate

* $\alpha:$ 10% type I error threshold

* $\beta:$ 10% type II error threshold
...
This output provides the key components ($r_1$, $r$, $n_1$, and $n$) as well as $EN(p_0)$ and $PET(p_0)$ for the selected optimal and minimax designs.
...
This output provides a visual representation of expected sample size ($EN(p_0)$) and maximum number of patients ($n$) under various scenarios. O represents the optimal design, and M represents the minimax design.
...
This outpoint includes the admissible design as well as the optimal and minimax designs. In addition to the design components seen earlier, columns `qLo` and `qHi` give the range of probability values for which the particular design is admissible.
...
When designing a single-stage trial, the key objective is to select appropriate values of $n$ (sample size) and $r$ (response threshold at the end of the study). The response rate considered "unacceptable" (at which point a treatment would not be recommended) and the response rate considered "desired" must be specified, and the values of $r$ and $n$ must be selected such that the desired error rate constraints are satisfied.

### Design Parameters and Constraints

The values of t $n$ (sample size) and $r$ (response threshold at the end of the study) depend on what response rate is considered "unacceptable" ($p_0:$; at which point a treatment would not be recommended), what response rate is considered "desired" ($p_1:$) as well as the desired error rate constraints ($\alpha:$, $\beta:$) needed to test the following hypotheses:
...
The trial employed a single-stage design. A disease control rate of 40% was considered unacceptable, and a disease control rate of 60% was considered desired. The  type I error threshold used was 10%. The target power used was 90%, equivalent to a 10% type II error threshold.

Summarizing the design parameters and constraints from above, we have:

* $p_0:$ 40% disease control rate

* $p_1:$ 60% disease control rate
...
{clinfun} offers several functions to determine sample sizes, effect sizes, etc based on Fisher's exact tests. Below is a table summarizing the functions based on Fisher's exact test.
...
*CAUTION:* As we can see below, when we use base R `power.prop.test()`, we obtain an incorrect results due to normal approximation.
