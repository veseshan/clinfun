...
This output provides the key design components ($r_1$, $r$, $n_1$, and $N$) as well as $EN(p_0)$ and $PET(p_0)$ for the selected optimal and minimax designs. As expected, the optimal design has a smaller expected sample size ($EN(p_0)$; about 29 vs. 31), whereas the minimax design has a smaller maximum sample size ($r$; 23 vs. 26).
...
This output provides a visual representation of expected sample size ($EN(p_0)$) and maximum number of patients ($r$) under various scenarios. O represents the optimal design, and M represents the minimax design.
