...
When designing a Simon's two-stage trial, the key objective is to select appropriate values of $r_1$ (response threshold at the end of stage 1), $r$ (final response threshold), $n_1$ (stage 1 sample size), and $n$ (total sample size; $n_1 + n_2$). The response rate considered "unacceptable" (at which point a treatment would not be recommended) and the response rate considered "desired" must be specified, and the values of $r_1$, $r$, $n_1$, and $n$ must be selected such that the desired error rate constraints are satisfied.
...
For this example, we will be taking a look at a phase II trial that assessed the efficacy of lenvatinib in patients with progressive, recurrent or metastatic adenoid cystic carcinoma (Tchekmedyian et al., 2019). Patients received lenvatinib 24 mg/day until disease progression, death, unacceptable toxicity, withdrawal of consent, or a decision based on the treating physician's discretion. Treatment response was specifically defined as overall response: either partial or complete response, as defined by RECIST version 1.1.
...
For this particular trial, Tchekmedyian et al. opted for the minimax design. Thus, 18 patients were enrolled in stage 1 ($n_1 = 18$). If no responses were observed ($r_1 \leq 0$ reduces to $r_1 = 0$ since the number of responses cannot be negative), then early termination would occur. Otherwise, if at least one response was observed ($r_1 > 0$), then 14 patients would be enrolled in stage 2 ($n_2 = n - n_1 = 32 - 18 = 14$). At the end of stage 2, if at least 4 responses had been observed in total ($r > 3$), then the treatment would be recommended; if 3 or fewer responses had been observed in total ($r \leq 3$), then the treatment would not be recommended.

Ultimately, Tchekmedyian et al. observed four responses among the 18 patients enrolled in stage 1 and thus proceeded to stage 2, in which an additional 14 patients were enrolled. One response was observed among the stage 2 patients. Thus, at the end of the trial, 5 responses had been observed in total, and lenvatinib was recommended for further development.
